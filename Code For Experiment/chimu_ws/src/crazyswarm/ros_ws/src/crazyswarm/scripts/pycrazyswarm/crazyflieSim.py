...
class Crazyflie:
    ...
    def __init__(self, id, initialPosition, timeHelper):
        ...
        self.state.pos = firm.mkvec(initialPosition[0],initialPosition[1],initialPosition[2])
    ...
    def enableCollisionAvoidance(self, others, ellipsoidRadii, bboxMin=np.repeat(-np.inf, 3), bboxMax=np.repeat(np.inf, 3), horizonSecs=1.0, maxSpeed=2.0):
        ...
        params.bboxMin = firm.mkvec(bboxMin[0],bboxMin[1],bboxMin[2])
        params.bboxMax = firm.mkvec(bboxMax[0],bboxMax[1],bboxMax[2])
    ...
    def goTo(self, goal, yaw, duration, relative = False, groupMask = 0):
        if self._isGroup(groupMask):
            ...
            self.mode = Crazyflie.MODE_HIGH_POLY
            firm.plan_go_to(self.planner, relative, firm.mkvec(goal[0],goal[1],goal[2]), yaw, duration, self.time())
    ...
    def cmdFullState(self, pos, vel, acc, yaw, omega):
        ...
        self.setState.pos = firm.mkvec(pos[0],pos[1],pos[2])
        self.setState.vel = firm.mkvec(vel[0],vel[1],vel[2])
        self.setState.acc = firm.mkvec(acc[0],acc[1],acc[2])
        self.setState.yaw = yaw
        self.setState.omega = firm.mkvec(omega[0],omega[1],omega[2])
    ...
    def integrate(self, time, disturbanceSize, maxVel):
        ...
        velocity = velocity + firm.mkvec(disturbance[0],disturbance[1],disturbance[2])

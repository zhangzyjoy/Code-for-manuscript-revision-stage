...
**Therefore, as the reviewers pointed out, there exists a research gap that the attitude controller proposed in the manuscript is not validated in the hardware platform. We are sorry about the fact that the proposed nonsingular Lie-algebra-based sliding mode attitude controller (NLSMAC) still remains in a stage of theoretical deduction and numerical simulation. We are working hard to find out a feasible approach to validate the whole closed-loop control scheme in future studies.** <br/><br/> 
...
![image](https://github.com/zhangzyjoy/Code-for-manuscript-revision-stage/blob/main/Pics/experiment_hardware_structure.png)  <br/><br/> 
...
```
5. Power on the crazyflie UAVs and use the terminal to choose the corresponding UAV nodes<br/>
```
cd /home/username/chimu_ws/src/crazyswarm/ros_ws/src/crazyswarm/scripts/
python3 chooser.py
```
Close the window of chooser and then run control scripts via ROS (Robot Operating System) to deploy the control scheme on the Crazyswarm UAVs. <br/><br/>
